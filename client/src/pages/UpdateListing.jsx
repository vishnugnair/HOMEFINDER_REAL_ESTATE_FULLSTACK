...
export default function UpdateListing() {
  ...
  const[formData,setFormData]=useState({
    imageUrls:[],
    name:'',
    description:'', address:'',type:'rent',bedrooms:1,bathrooms:1,regularPrice:50,discountPrice:0,
    offer:false,
  ...
  const handleImageSubmit=async()=>{
      if(files.length>0&&files.length+formData.imageUrls.length<7){ 
        ...
        setImageUploadError('Can upload only six images per listing');
        setUploading(false);
      }
  }
   ...
   const handleRemoveImage=(index)=>{
    setFormData({
      ...formData,
      imageUrls:formData.imageUrls.filter((_,i)=>i!==index),//no braces for function bcs only one line.
    })
   }
   const handleChange=(e)=>{
    ...
    if(e.target.id==='parking'||e.target.id==='furnished'||e.target.id==='offer'){
      setFormData({
        ...formData,
        [e.target.id]:e.target.checked //bcs it is a checkbox and bool type of variable.
      })
    } 
   ...
   const handleSubmit=async(e)=>{
    ...
    if(+formData.regularPrice<+formData.discountPrice){return setError('Discounted price must be less than original price')} //+ is added to prevent num string confusion.
    try {
      setLoading(true);
      setError(false);
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
     <h1 className='text-3xl font-bold text-center my-7'>Updating a listing</h1>
