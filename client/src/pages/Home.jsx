  ...
  useEffect(()=>{
   const fetchOfferListings=async()=>{
    try {
      ...
      fetchSRentListings();//called in this manner bcs to keep good order
    } catch (error) {
      console.log(error);
    }
   ...
   const fetchSRentListings=async()=>{
    try {
      const res=await fetch("/api/listing/get?type=rent&limit=4");
      const data=await res.json();
      setRentListings(data);
      fetchSaleListings();//called in this manner bcs to keep good order
    } catch (error) {
      console.log(error);
    }
   }
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-6 p-28 px-3 max-w-6xl mx-auto">
        <h1 className="text-slate-700 font-bold text-3xl lg:text-6xl"> Real Estate  
        <span className="text-slate-900"> marketplace</span> <br/> to make your life easy! </h1>

        <div className="text-gray-400 text-xs sm:text-sm">
          This is the best place to find your next stay!
          <br/>
